      ...
      <article className="rounded-lg border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 flex flex-wrap gap-2">
          <button type="button" className="rounded-md bg-slate-900 px-3 py-2 text-sm text-white" onClick={async () => {
            const response = await fetch("/networking/session/start", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ userId: DEFAULT_USER_ID, scenarioId, framework, persona }) });
          ...
          <button type="button" className="rounded-md bg-emerald-700 px-3 py-2 text-sm text-white" onClick={async () => {
            if (!sessionId) return;
            const response = await fetch("/networking/session/end", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ sessionId, userId: DEFAULT_USER_ID }) });
            const payload = await response.json();
            if (payload.ok) {
              setResult(payload);
